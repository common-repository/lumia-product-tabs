 ...
 *
 * Copyright: (c) 2015-2019 Jinesh.P.V (email: jinuvijay5@gmail.com)
 *
...
class LumiaProductTabs {
	...
	public function lumia_product_admin_tab_content_html() {
		...
		$i = 0;
		echo '<div id="lumia_product_tabs" class="panel wc-metaboxes-wrapper woocommerce_options_panel"><div class="lumia_product_tabs">';
		
		foreach ( $lumia_product_tab as $tabObj ) {
			
			// display the custom tab panel
			echo 	'<div class="lumia_product_tab_wrapper"><div class="lumia_product_tab_inner">';
			...
			self::lumia_tab_content_textarea_html( array( 'id' => '_wc_lumia_product_tabs_content_' . $i, 'name' => '_wc_lumia_product_tabs_content[' . $i . ']', 'label' => __( 'Description', TEXT_DOMAIN ), 'value' => $tabObj['tab_content'] ) );
			echo 	'</div>';
			$i++;
			echo '<div class="action_block"><a href="javascript:;" class="button deletetab button-small">Delete tab</a></div></div>';
		}
		for( $k = count( $lumia_product_tab ); $k < 16; $k++ ){
			echo '<div class="lumia_product_tab_wrapper hidden_editor_' . $k . ' hide">		
					<div class="lumia_product_tab_inner">
						<p class="form-field _wc_lumia_product_tabs_title_' . $k . '_field">
							<label for="_wc_lumia_product_tabs_title_lwpt">Tab Name</label>
							<input type="text" id="_wc_lumia_product_tabs_title_' . $k . '" name="_wc_lumia_product_tabs_title[' . $k . ']" class="wc_lumia_product_tabs_title">
						</p>
						<div class="form-field _wc_lumia_product_tabs_content_lwpt_field">'; 
							wp_editor( "", "wc_lumia_product_tabs_" . $k, array(
																							'textarea_rows' => 8,
																							'tinymce'   => true,
																							'quicktags' => true,
																							'media_buttons' => true,
																							'editor_height' => '200px',
																							'textarea_name' => '_wc_lumia_product_tabs_content[' . $k . ']'
																						)
																					);
			echo 		'</div>
					</div>
					<div class="action_block"><a href="javascript:;" class="button deletetab button-small">Delete tab</a></div>
				</div>';
	}
		echo '</div><div class="action_block add_box"><a href="javascript:;" class="button addtab button-small button-primary">Add new tab</a></div></div>';
				?>
				<style type="text/css">
				.woocommerce_options_panel .wc_lumia_product_tabs_title { width: 100% !important; } .lumia_product_tab_wrapper { background-color: #f5f5f5; margin: 10px; padding: 10px; } .hide { display: none; } .action_block { margin-top: 10px; } .add_box { margin: 0 10px 10px;} .wp-editor-area { border-color: #fff; } .lumia_product_tabs .wp-switch-editor { display: none; }
			  </style>
		<?php
	}
			...
			if ( $_POST['_wc_lumia_product_tabs_title'] ) { 
				...
				for( $i = 0; $i < count( $_POST['_wc_lumia_product_tabs_title'] ) ; $i++ ) {
					...
					if ( strlen( $_POST['_wc_lumia_product_tabs_title'][$i] ) != strlen( utf8_encode( $_POST['_wc_lumia_product_tabs_title'][$i] ) ) ) {
						...
						$tab_id = strtolower( $_POST['_wc_lumia_product_tabs_title'][$i] );
					...
					if( !empty( $_POST['_wc_lumia_product_tabs_title'][$i] ) && !empty( $_POST['_wc_lumia_product_tabs_content'][$i] ) ) {
						$tab_data[] = array( 
	...
	private function lumia_tab_content_textarea_html( $field ) {
		...
		<div class="form-field <?php echo $field['id'];?>_field">
			<?php wp_editor( $field['value'], $field['id'], $settings ); ?>
		</div>
		<?php
	}
